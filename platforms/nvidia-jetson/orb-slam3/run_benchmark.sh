...
# ORB-SLAM3 Benchmark Script for NVIDIA Jetson Orin NX
...
start_monitoring() {
    ...
    # Start tegrastats monitoring (if available)
    if command -v tegrastats >/dev/null 2>&1; then
        tegrastats --logfile "$RESULTS_DIR/tegrastats_$(date +%Y%m%d_%H%M%S).log" &
        TEGRASTATS_PID=$!
        echo $TEGRASTATS_PID > "$RESULTS_DIR/tegrastats.pid"
        log "Started tegrastats monitoring (PID: $TEGRASTATS_PID)"
    fi
    
    # Start jtop monitoring (if available)
    if command -v jtop >/dev/null 2>&1; then
        jtop --write "$RESULTS_DIR/jtop_$(date +%Y%m%d_%H%M%S).json" &
        JTOP_PID=$!
        echo $JTOP_PID > "$RESULTS_DIR/jtop.pid"
        log "Started jtop monitoring (PID: $JTOP_PID)"
    fi
}
...
stop_monitoring() {
    ...
    # Stop tegrastats
    if [ -f "$RESULTS_DIR/tegrastats.pid" ]; then
        TEGRASTATS_PID=$(cat "$RESULTS_DIR/tegrastats.pid")
        kill $TEGRASTATS_PID 2>/dev/null || true
        rm "$RESULTS_DIR/tegrastats.pid"
        log "Stopped tegrastats monitoring"
    fi
    
    # Stop jtop
    if [ -f "$RESULTS_DIR/jtop.pid" ]; then
        JTOP_PID=$(cat "$RESULTS_DIR/jtop.pid")
        kill $JTOP_PID 2>/dev/null || true
        rm "$RESULTS_DIR/jtop.pid"
        log "Stopped jtop monitoring"
    fi
}
...
ensure_quiescent_state() {
    ...
    while (( $(echo "$CPU_USAGE > 20" | bc -l) )); do
...
run_single_benchmark() {
    ...
    timeout 300 ./Examples/Monocular-Inertial/mono_inertial_euroc \
...
run_benchmark_suite() {
    ...
    echo "ORB-SLAM3 Benchmark Results - NVIDIA Jetson Orin NX" > "$RESULTS_DIR/summary.txt"
...
def main():
    ...
    print("\n" + "="*50)
    print("ORB-SLAM3 Performance Analysis - NVIDIA Jetson Orin NX")
    print("="*50)
    ...
    with open(results_dir / 'detailed_analysis.txt', 'w') as f:
        f.write("ORB-SLAM3 Detailed Performance Analysis\n")
        f.write("="*40 + "\n\n")
    ...
    ax1.set_title('ORB-SLAM3 Throughput per Run')
    ...
    ax2.set_title('ORB-SLAM3 P99 Latency per Run')
...
# Main execution
main() {
    log "Starting ORB-SLAM3 benchmark for NVIDIA Jetson Orin NX..."
