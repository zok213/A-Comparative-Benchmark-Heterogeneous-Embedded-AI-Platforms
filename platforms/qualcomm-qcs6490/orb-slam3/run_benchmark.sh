...
# ORB-SLAM3 Benchmark Script for Qualcomm QCS6490
...
# Optimize QCS6490 performance settings
optimize_qcs6490() {
    log "Optimizing QCS6490 performance settings..."
    ...
    # Disable CPU hotplug for consistency
    ...
    success "QCS6490 performance optimizations applied"
}
...
#!/bin/bash
while true; do
    echo "$(date): $(cat /sys/devices/system/cpu/cpu*/cpufreq/scaling_cur_freq 2>/dev/null | tr '\n' ' ')"
    sleep 1
done
EOF
    chmod +x "$RESULTS_DIR/monitor_cpu.sh"
    "$RESULTS_DIR/monitor_cpu.sh" > "$RESULTS_DIR/cpu_freq_$(date +%Y%m%d_%H%M%S).log" 2>&1 &
    CPU_MON_PID=$!
    echo $CPU_MON_PID > "$RESULTS_DIR/cpu_monitor.pid"
    log "Started CPU frequency monitoring (PID: $CPU_MON_PID)"
}
...
stop_monitoring() {
    ...
    # Stop CPU monitoring
    if [ -f "$RESULTS_DIR/cpu_monitor.pid" ]; then
        CPU_MON_PID=$(cat "$RESULTS_DIR/cpu_monitor.pid")
        kill $CPU_MON_PID 2>/dev/null || true
        rm "$RESULTS_DIR/cpu_monitor.pid"
        log "Stopped CPU frequency monitoring"
    fi
}
...
run_single_benchmark() {
    ...
    # Use taskset to bind to specific CPU cores for consistency
...
run_benchmark_suite() {
    ...
    echo "ORB-SLAM3 Benchmark Results - Qualcomm QCS6490" > "$RESULTS_DIR/summary.txt"
...
def main():
    ...
    print("\n" + "="*50)
    print("ORB-SLAM3 Performance Analysis - Qualcomm QCS6490")
    print("="*50)
    ...
    with open(results_dir / 'detailed_analysis.txt', 'w') as f:
        f.write("ORB-SLAM3 Detailed Performance Analysis - Qualcomm QCS6490\n")
        f.write("="*60 + "\n\n")
    ...
    ax1.set_title('ORB-SLAM3 Throughput per Run (QCS6490)')
    ...
    ax2.set_title('ORB-SLAM3 P99 Latency per Run (QCS6490)')
...
# Main execution
main() {
    log "Starting ORB-SLAM3 benchmark for Qualcomm QCS6490..."
